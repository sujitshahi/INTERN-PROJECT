...
const CULTURAL_PILLS = [
  { id: "creative-arts", icon: Palette, label: "Creative Arts", color: "bg-pink-100 text-pink-700 border-pink-200" },
  { id: "music-movement", icon: Music, label: "Music & Movement", color: "bg-purple-100 text-purple-700 border-purple-200" },
  { id: "annual-festivals", icon: Sparkles, label: "Annual Festivals", color: "bg-amber-100 text-amber-700 border-amber-200" },
  { id: "community-care", icon: HeartHandshake, label: "Community Care", color: "bg-emerald-100 text-emerald-700 border-emerald-200" },
];
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 gap-3 pt-2 max-w-lg">
              {CULTURAL_PILLS.map((pill) => {
                const Icon = pill.icon;
                return (
                  <m.div
                    key={pill.id}
                    whileHover={{ scale: 1.03 }}
                    className={`flex items-center gap-3 p-3 rounded-2xl border ${pill.color} shadow-xs transition-transform`}
                  >
